                    ...
                    for(Instruction &I : BB){
                        ...
                        if(callInstr){
                            ...
                            if (calledFunc && !calledFunc->isIntrinsic() && !calledFunc->isDeclaration()){           
                                ...
                                InlineAsm *asmCode = InlineAsm::get(FunctionType::get(Type::getVoidTy(Mcontext), false), Tname +"_" + to_string(cnt*4)  +"_FORPUSH:;movt lr, "+ to_string(cnt*4) +";movw lr, 0x080C;push {lr}", "", true,false);
                                builder.CreateCall(asmCode);
                                cnt++;
                            }
                        }
